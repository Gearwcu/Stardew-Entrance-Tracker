...
function OnMouseMove(event) {
    ...
    // TUNIC pack: added to get hover text on warps
    else {
        let info = null;
        info = GetWarp(mouse_position);
        if (info && info.type == TYPE_WARP) {
            current_hovering_mark = info;
        }
    }
}
...
function ChangeWarpOffline(current_game, location, warp, link_type, link_location, link, modifier) {
    ...
    // Save the warp
    let current_warp = current_game.warps[location][warp];
    if (current_warp.link_type != link_type || current_warp.link_location != link_location || current_warp.link != link) {
        let old_link = null;
        if (current_warp.link_type == LINKTYPE_MARK) {
            old_link = current_warp.link;
        }
        current_warp.link_type     = link_type;
        current_warp.link_location = link_location;
        current_warp.link          = link;
        if (old_link) {
            AddToIcon(current_game, old_link, -1, location, TYPE_MARK);
        }
        if (link_type == LINKTYPE_MARK) {
            AddToIcon(current_game, link, 1, location, TYPE_MARK);
        }
    }
    
    if (modifier == "null") { modifier = null };
    if (current_warp.modifier != modifier) {
        let old_modifier = null;
        if (current_warp.modifier || current_warp.modifier == "null") { old_modifier = current_warp.modifier; }
        current_warp.modifier = modifier;
